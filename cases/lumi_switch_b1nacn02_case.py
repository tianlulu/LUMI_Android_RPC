...
from handle.lumi_switch_b1nacn02_handle import Switch_B1nacn02_Handle
...
class Switch_B1nacn02_Case(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.lumi_switch_b1nacn02_handle = Switch_B1nacn02_Handle()
    ...
    def test_case1(self):
        '''找出Aqara墙壁开关(零火线单键版)'''
        global room_exists
        room_exists = self.lumi_switch_b1nacn02_handle.click_room_element()
        self.assertTrue(room_exists,'滑动到底部仍然没有找到房间')

        global device_exists
        device_exists = self.lumi_switch_b1nacn02_handle.click_device_element()
    ...
    def test_case2(self):
        ...
        is_no_network = self.lumi_switch_b1nacn02_handle.get_fail_toast('请求失败，请检查网络')
        self.assertFalse(is_no_network,'无网络状态，请联网后重试')

        # 判断设备是否离线状态
        global is_offline
        is_offline = self.lumi_switch_b1nacn02_handle.is_offline_element()
    ...
    def test_case3(self):
        ...
        rgba = self.lumi_switch_b1nacn02_handle.get_light_bulb_element_rgba()
        # print('得到的灯泡颜色为:',rgba)
        self.assertTrue(self.lumi_switch_b1nacn02_handle.click_on_off_light_element(), '找不到开关按钮')
        if rgba == gray_rgba:
    ...
    def test_case4(self):
        ...
        if room_exists == False:
            print('房间不存在')
            self.assertTrue(self.lumi_switch_b1nacn02_handle.scroll_to_top(), "下滑到最后没有找到+按钮")
            return

        if device_exists == False:
            print('设备不存在')
            self.assertTrue(self.lumi_switch_b1nacn02_handle.click_room_list_back_element(),'返回到房间列表的箭头按钮不存在')
            self.assertTrue(self.lumi_switch_b1nacn02_handle.scroll_to_top(),'下滑到最后没有找到+按钮')
            return
        ...
        if is_offline:
            print('设备离线状态')
            self.assertTrue(self.lumi_switch_b1nacn02_handle.click_offline_close(),'离线框右上角关闭按钮不存在')
            self.expected_conditions()
            return
    ...
    def expected_conditions(self):
        self.assertTrue(self.lumi_switch_b1nacn02_handle.click_plugin_back_homepage_element(), '返回到房间里面的箭头按钮不存在')
        self.assertTrue(self.lumi_switch_b1nacn02_handle.click_room_list_back_element(), '返回到房间列表的箭头按钮不存在')
        self.assertTrue(self.lumi_switch_b1nacn02_handle.scroll_to_top(), "下滑到最后没有找到+按钮")
