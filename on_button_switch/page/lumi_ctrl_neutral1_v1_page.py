...
class Ctrl_Neutral1_V1_Page(One_Button_Wall_Switch_Page):
    def __init__(self):
    ...
    Aqara墙壁开关(零火线单键版)--房间
    '''
    def get_ctrl_neutral1_v1_room_element(self):
    ...
    '''
    def get_ctrl_neutral1_v1_device_element(self):
        device_name = self.custom_name.get_ctrl_neutral_v1_device_name()
        if device_name != "":
            return self.get_custom_element(device_name, 1, 0.5)
        else:
            return self.my_local.get_element('home_ctrl_neutral1_v1', self.ini_element, self.ini_element_file_path)
