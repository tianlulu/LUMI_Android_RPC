...
class Switch_B1lacn02_Page(One_Button_Wall_Switch_Page):
    def __init__(self):
    ...
    Aqara墙壁开关(零火线单键版)--房间
    '''
    def get_switch_b1lacn02_room_element(self):
        room_name = self.custom_name.get_switch_b1nacn02_room_name()
        if room_name != "":
    ...
    '''
    def get_switch_b1lacn02_device_element(self):
        device_name = self.custom_name.get_switch_b1nacn02_device_name()
        if device_name != "":
            return self.get_custom_element(device_name, 1, 0.5)
        else:
            return self.my_local.get_element('home_switch_b1lacn02', self.ini_element, self.ini_element_file_path)
