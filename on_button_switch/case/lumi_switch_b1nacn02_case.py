...
from on_button_switch.handle.lumi_switch_b1nacn02_handle import Switch_B1nacn02_Handle
...
'''
Aqara智能墙壁开关 D1（零火线单键版）测试用例
'''
class Switch_B1nacn02_Case(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        print('开始Switch_B1nacn02_Case')
        cls.one_button_wall_switch_data = One_Button_Wall_Switch_Data()
        cls.switch_b1nacn02_handle = Switch_B1nacn02_Handle()
        cls.one_button_wall_switch_business = On_Button_Wall_Switch_Business(cls.switch_b1nacn02_handle)
    ...
    def test_case1(self):
        '''找出Aqara墙壁开关(零火线单键版)'''
        global room_exists
        room_exists = self.switch_b1nacn02_handle.click_switch_b1nacn02_room_element()
        self.assertTrue(room_exists, '滑动到底部仍然没有找到房间')

        global device_exists
        device_exists = self.switch_b1nacn02_handle.click_switch_b1nacn02_device_element()
        self.assertTrue(device_exists, '滑动到底部仍然没有找到设备或者点击设备无响应')

        global is_no_network
        is_no_network = self.switch_b1nacn02_handle.get_fail_toast('请求失败，请检查网络')
        self.assertFalse(is_no_network, '无网络状态，请联网后重试')

        # 判断设备是否离线状态
        global is_offline
        is_offline = self.switch_b1nacn02_handle.is_offline_element()
    ...
    def test_case3(self):
        ...
        rgba = self.switch_b1nacn02_handle.get_light_bulb_element_rgba()
        self.assertTrue(self.switch_b1nacn02_handle.click_on_off_light_element(), '找不到开关按钮')
    ...
    def test_case6(self):
        ...
        '''
        self.determine_current_status_of_the_device()
        self.assertTrue(self.switch_b1nacn02_handle.click_current_day_electricity_element())
        self.one_button_wall_switch_business.get_the_correct_electricity_data(self.one_button_wall_switch_data, True)

    def test_case7(self):
        '''点击"月":统计当月用电'''
        self.determine_current_status_of_the_device()
        self.assertTrue(self.switch_b1nacn02_handle.click_month_degree_element())
    ...
    def test_case8(self):
        ...
        '''
        self.determine_current_status_of_the_device()
        self.assertTrue(self.switch_b1nacn02_handle.click_plugin_back_homepage_element(), '返回到插件首页的箭头按钮不存在')
        self.assertTrue(self.switch_b1nacn02_handle.click_current_month_electricity_element())
        self.one_button_wall_switch_business.get_the_correct_electricity_data(self.one_button_wall_switch_data, False)

    def test_case9(self):
        '''点击"日":统计当日电量'''
        self.determine_current_status_of_the_device()
        self.assertTrue(self.switch_b1nacn02_handle.click_day_degree_element())
        self.one_button_wall_switch_business.get_the_correct_electricity_data(self.one_button_wall_switch_data, True)
    ...
    def test_case10(self):
        ...
        self.determine_current_status_of_the_device()
        self.assertTrue(self.switch_b1nacn02_handle.click_plugin_back_homepage_element(), '返回到插件首页的箭头按钮不存在')
        self.assertTrue(self.switch_b1nacn02_handle.click_current_power_element())
        self.one_button_wall_switch_business.get_the_correct_power_data(self.one_button_wall_switch_data, True)

    def test_case11(self):
        '''点击"周":统计本周功率'''
        self.determine_current_status_of_the_device()
        self.assertTrue(self.switch_b1nacn02_handle.click_week_degree_element())
    ...
    def test_case28(self):
        '''转无线开关设置：获取转无线开关状态'''
        self.determine_current_status_of_the_device()
        assert self.switch_b1nacn02_handle.click_title_bar_return_button_element() == True, '定时首页返回按钮不存在'
        self.assertTrue(self.switch_b1nacn02_handle.click_turn_wireless_switch_settings_element(), '转无线开关设置cell元素不存在')
        self.one_button_wall_switch_data.get_wireless_switch_status()

    def test_case29(self):
        '''转无线开关设置：打开/关闭转无线开关按钮'''
        self.determine_current_status_of_the_device()
        self.switch_b1nacn02_handle.swipe_with('up')
        # 蓝色
        blue_rgba = (60, 168, 192, 255)
        rgba = self.switch_b1nacn02_handle.get_switch_element_rgba()
        self.assertTrue(self.switch_b1nacn02_handle.click_wireless_switch_function_switching_element(), '没有获取到转无线开关的按钮')
        print('rgba:', rgba)
        if rgba == blue_rgba:
    ...
    def test_case34(self):
        ...
        if room_exists == False:
            print('房间不存在')
            self.assertTrue(self.switch_b1nacn02_handle.scroll_to_top(), "下滑到最后没有找到+按钮")
            return

        if device_exists == False:
            print('设备不存在')
            self.assertTrue(self.switch_b1nacn02_handle.click_room_list_back_element(), '返回到房间列表的箭头按钮不存在')
            self.assertTrue(self.switch_b1nacn02_handle.scroll_to_top(), '下滑到最后没有找到+按钮')
            return
        ...
        if is_offline:
            print('设备离线状态')
            self.assertTrue(self.switch_b1nacn02_handle.click_offline_close(), '离线框右上角关闭按钮不存在')
            self.expected_conditions()
            return
    ...
    def expected_conditions(self):
        self.assertTrue(self.switch_b1nacn02_handle.click_plugin_back_homepage_element(), '返回到房间里面的箭头按钮不存在')
        self.assertTrue(self.switch_b1nacn02_handle.click_room_list_back_element(), '返回到房间列表的箭头按钮不存在')
        self.assertTrue(self.switch_b1nacn02_handle.scroll_to_top(), "下滑到最后没有找到+按钮")
...
def get_suite():
    ...
    suite.addTest(Switch_B1nacn02_Case('test_case1'))
    # suite.addTest(Switch_B1nacn02_Case('test_case2'))
    # suite.addTest(Switch_B1nacn02_Case('test_case3'))
    # suite.addTest(Switch_B1nacn02_Case('test_case4'))
    # suite.addTest(Switch_B1nacn02_Case('test_case5'))
    suite.addTest(Switch_B1nacn02_Case('test_case6'))
    suite.addTest(Switch_B1nacn02_Case('test_case7'))
    suite.addTest(Switch_B1nacn02_Case('test_case8'))
    suite.addTest(Switch_B1nacn02_Case('test_case9'))

    # suite.addTest(Switch_B1nacn02_Case('test_case10'))
    # suite.addTest(Switch_B1nacn02_Case('test_case11'))
    suite.addTest(Switch_B1nacn02_Case('test_case12'))
    suite.addTest(Switch_B1nacn02_Case('test_case13'))

    # suite.addTest(Switch_B1nacn02_Case('test_case14'))
    # suite.addTest(Switch_B1nacn02_Case('test_case15'))
    # suite.addTest(Switch_B1nacn02_Case('test_case16'))
    # suite.addTest(Switch_B1nacn02_Case('test_case17'))
    # suite.addTest(Switch_B1nacn02_Case('test_case18'))
    # suite.addTest(Switch_B1nacn02_Case('test_case19'))
    #
    # suite.addTest(Switch_B1nacn02_Case('test_case20'))
    # suite.addTest(Switch_B1nacn02_Case('test_case21'))
    # suite.addTest(Switch_B1nacn02_Case('test_case22'))
    # suite.addTest(Switch_B1nacn02_Case('test_case23'))
    # suite.addTest(Switch_B1nacn02_Case('test_case24'))
    #

    # suite.addTest(Switch_B1nacn02_Case('test_case25'))
    # suite.addTest(Switch_B1nacn02_Case('test_case26'))
    suite.addTest(Switch_B1nacn02_Case('test_case27'))
    suite.addTest(Switch_B1nacn02_Case('test_case28'))
    suite.addTest(Switch_B1nacn02_Case('test_case29'))
    suite.addTest(Switch_B1nacn02_Case('test_case30'))
    suite.addTest(Switch_B1nacn02_Case('test_case31'))
    suite.addTest(Switch_B1nacn02_Case('test_case32'))
    suite.addTest(Switch_B1nacn02_Case('test_case33'))
    suite.addTest(Switch_B1nacn02_Case('test_case34'))
