'''
找room房间-找到--点击进入房间（需要校验发现附近设备--家庭共享--门锁报警弹窗）
          找不到--'没有找到房间'（需要校验发现附近设备--家庭共享--门锁报警弹窗）
找到设备--找到点击--进入插件首页---加载成功进入插件页面 （需要校验离线-升级-授权弹框）
                                加载失败一直停留在loading页面
         找到点击--点击没反应（最好重启app、重启app 还未做）
         找不到--'没有找到设备'
'''
from on_button_switch.on_button_wall_switch_handle import One_Button_Wall_Switch_Handle
from on_button_switch.page.lumi_ctrl_ln1_aq1_page import Ctrl_Ln1_Aq1_Page
import time
class Ctrl_Ln1_Aq1_Handle(One_Button_Wall_Switch_Handle):
    def __init__(self):
        self.page_model = Ctrl_Ln1_Aq1_Page()
    ...
    step2:找房间
    '''
    def click_ctrl_ln1_aq1_room_element(self):
        self.get_title_bar_room_element()
        i = 0
        count = 7
        while i < count:
            # 找房间
            element = self.page_model.get_ctrl_ln1_aq1_room_element()
    ...
    step2:找设备
    '''
    def click_ctrl_ln1_aq1_device_element(self):
        i = 0
        count = 5
        while i < count:
            # 找设备
            element = self.page_model.get_ctrl_ln1_aq1_device_element()
            if element != None:
                element.click()
                # 判断是否成功进入插件
                time.sleep(2)
