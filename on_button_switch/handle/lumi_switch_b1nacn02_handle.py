...
from on_button_switch.page.lumi_switch_b1nacn02_page import Switch_B1nacn02_Page
import time
class Switch_B1nacn02_Handle(One_Button_Wall_Switch_Handle):
    def __init__(self):
        self.page_model = Switch_B1nacn02_Page()
    ...
    step2:找房间
    '''
    def click_switch_b1nacn02_room_element(self):
        self.get_title_bar_room_element()
        i = 0
        count = 7
        while i < count:
            # 找房间
            element = self.page_model.get_switch_b1nacn02_room_element()
    ...
    step2:找设备
    '''
    def click_switch_b1nacn02_device_element(self):
        i = 0
        count = 5
        while i < count:
            # 找设备
            element = self.page_model.get_switch_b1nacn02_device_element()
            if element != None:
                element.click()
                # 判断是否成功进入插件
                time.sleep(2)
