...
from on_button_switch.page.lumi_ctrl_ln1_v1_page import Ctrl_Ln1_V1_Page
import time
class Ctrl_Ln1_V1_Handle(One_Button_Wall_Switch_Handle):
    def __init__(self):
        self.page_model = Ctrl_Ln1_V1_Page()
    ...
    step2:找房间
    '''
    def click_ctrl_ln1_v1_room_element(self):
        self.get_title_bar_room_element()
        i = 0
        count = 7
        while i < count:
            # 找房间
            element = self.page_model.get_ctrl_ln1_v1_room_element()
    ...
    step2:找设备
    '''
    def click_ctrl_ln1_v1_device_element(self):
        i = 0
        count = 5
        while i < count:
            # 找设备
            element = self.page_model.get_ctrl_ln1_v1_device_element()
            if element != None:
                element.click()
                # 判断是否成功进入插件
                time.sleep(2)
